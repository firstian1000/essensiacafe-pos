        ...
        <form action="{{ route('stocks.update', $stockItem) }}" method="POST">
            ...
            <div class="form-row">
                    ...
                    <div class="stock-name-actions">
                        <select name="name" class="form-control js-stock-name-select" required>
                            @foreach($stockNames as $stockName)
                                <option value="{{ $stockName }}" {{ old('name', $stockItem->name) === $stockName ? 'selected' : '' }}>{{ $stockName }}</option>
                            @endforeach
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="stock" class="form-control" value="{{ old('stock', $stockItem->stock) }}" min="0" placeholder="Masukkan jumlah stok" required>
            ...
            <div class="form-row">
                <div class="form-group">
                    <label class="form-label">Status Stok <span class="text-danger">*</span></label>
                    <select name="status" class="form-control" required>
                        <option value="1" {{ old('status', $stockItem->status) ? 'selected' : '' }}>Aktif</option>
                        <option value="0" {{ !old('status', $stockItem->status) ? 'selected' : '' }}>Habis</option>
                    </select>
